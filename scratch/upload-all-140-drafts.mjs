...
async function main() {
  ...
  // Fetch ALL 140 jobs in the pipeline
  ...
  console.log(`🚀 Found ${res.rows.length} total jobs in pipeline to generate Gmail drafts for.`);
  ...
  for (let i = 0; i < res.rows.length; i++) {
    ...
    let cvPdfPath = null;
    if (job.cv_html) {
      // Remove any lingering Projects section from HTML before rendering
    ...
    // Fall back to pre-generated CV if rendering dynamic CV failed
...
<body>
  ...
  <div>${job.cover_letter.replace(/\n/g, '<br>')}</div>
    ...
    try {
      ...
      count++;
      console.log(`[${count}/140] ✅ ${company} — ${role} (${attachments.length} attachments: Resume + Cover Letter)`);
    } catch (e) {
      errorCount++;
  ...
  await browser.close();
  console.log(`\n══════════════════════════════════════════════════`);
  console.log(`🎉 COMPLETED: Successfully uploaded ${count}/140 Gmail drafts!`);
  console.log(`══════════════════════════════════════════════════`);
