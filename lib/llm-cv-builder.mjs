...
function buildCoverLetterPrompt(jdText, profile) {
  return `You are an expert ATS optimization AI. Your job is to write a highly tailored cover letter for a candidate that highlights their skills and traits that perfectly match the provided Job Description.
...
RULES:
1. Write the cover letter from the perspective of the candidate, highlighting how their experience uniquely fits the JD.
2. Keep it concise, professional, and impactful (around 3 paragraphs).
3. Do NOT invent or hallucinate experiences or companies that are not in the profile.
4. Output ONLY the raw text of the cover letter. Do not include markdown formatting or placeholder brackets. Do not include the date or address block at the top, just start with "Dear Hiring Manager," (or similar) and end with the candidate's name.`;
}

export async function generateLLMCoverLetter(profile, jdText) {
  const providers = profile.llm_providers || [];
  if (providers.length === 0) {
    throw new Error("No LLM providers configured in profile.yml under 'llm_providers'");
  }

  const prompt = buildCoverLetterPrompt(jdText, profile);
  ...
  for (const provider of providers) {
    try {
      ...
      console.log(`[LLM Waterfall] Attempting to use ${provider.name} (${provider.model}) for Cover Letter...`);
      ...
      console.log(`[LLM Waterfall] Successfully generated Cover Letter using ${provider.name}!`);
      return content.trim();
      
    } catch (error) {
      console.error(`[LLM Waterfall] ${provider.name} failed for Cover Letter:`, error.message);
    }
  }

  throw new Error("All LLM providers in the waterfall failed to generate the Cover Letter.");
}
