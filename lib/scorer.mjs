...
const GEMINI_MODEL = process.env.GEMINI_SCORING_MODEL || 'gemini-3.1-flash-lite';
...
const LLM_PROVIDERS = [
  ...
  {
    // NVIDIA NIM speaks the OpenAI-compatible dialect and hosts full-size
    // instruct models on a free developer tier. It sits second because it is
    // the only provider in this chain with neither an exhausted daily quota
    // (Gemini) nor a token-per-minute ceiling that a single scan cycle blows
    // through (Groq's 8k/min). llama-3.3-70b was chosen over the nemotron and
    // llama-3.1 variants because it is the one that reproduces the required
    // OUTPUT FORMAT exactly: 3.1-70b separates DIMENSIONS with commas, which
    // the DIMENSIONS regex below does not accept, and silently loses every
    // per-dimension score.
    name: 'nvidia',
    label: () => `NVIDIA (${process.env.NVIDIA_MODEL || 'meta/llama-3.3-70b-instruct'})`,
    key: () => process.env.NVIDIA_API_KEY || '',
    call: (key, prompt, maxTokens) => openaiCompatCall(
      'https://integrate.api.nvidia.com/v1', key,
      process.env.NVIDIA_MODEL || 'meta/llama-3.3-70b-instruct', prompt, {}, maxTokens),
  },
  {
    name: 'groq',
