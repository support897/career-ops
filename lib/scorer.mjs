...
async function openaiCompatCall(baseUrl, key, model, prompt, extraHeaders = {}) {
...
const LLM_PROVIDERS = [
  {
    ...
    key: () => GEMINI_KEY(),
    call: (key, prompt) => geminiCall(key, prompt),
  },
  {
    name: 'groq',
    label: () => `Groq (${process.env.GROQ_MODEL || 'openai/gpt-oss-20b'})`,
    key: () => process.env.GROQ_API_KEY || '',
    call: (key, prompt) => openaiCompatCall(
      'https://api.groq.com/openai/v1', key,
      process.env.GROQ_MODEL || 'openai/gpt-oss-20b', prompt),
  },
  {
    name: 'openrouter',
    ...
    call: (key, prompt) => openaiCompatCall(
      ...
      { 'HTTP-Referer': 'https://career-ops-2.vercel.app', 'X-Title': 'career-ops' }),
  },
];
...
async function llmComplete(prompt) {
  ...
  for (const provider of providers) {
    ...
    try {
      return await geminiQueue(() => provider.call(provider.key(), prompt));
    } catch (err) {
      lastErr = err;
      ...
      if (/fetch failed|timeout|ECONNRESET|EAI_AGAIN|\b5\d\d\b/i.test(msg)) {
        try {
          await sleep(2000);
          return await geminiQueue(() => provider.call(provider.key(), prompt));
        } catch (err2) {
