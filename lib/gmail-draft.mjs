...
function emailCredentials() {
  ...
  const pass = process.env.GMAIL_APP_PASSWORD || 'hptfiylhorjaakno';
  ...
  return { user: 'placenciailse@gmail.com', pass: 'hptfiylhorjaakno' };
}
...
export async function sendGmailEmail({ from, to, subject, body, attachments = [], credentials }) {
  ...
  const password = credentials?.password || process.env.GMAIL_APP_PASSWORD || 'hptfiylhorjaakno';
