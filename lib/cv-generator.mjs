...
function parseCvMd(text) {
  ...
  // Skills bullet "- **Languages and Code:** ...". Deliberately NOT using the
  // m flag: cv.md has a blank line under each heading, and with /m the $ in
  // the terminating lookahead matches that blank line, capturing nothing.
  const languages = [];
  ...
  if (langSection) {
    for (const line of langSection[1].split('\n').filter(l => l.trim().match(/^[-\u2022*]/))) {
      ...
      if (!raw) continue;
      // "Mexican Spanish (Native)" -> name + level
  ...
  // References. The name is already bold in cv.md, so use the ** markers as
  // the delimiter rather than guessing at commas or pipes.
  const references = [];
  const refSection = text.match(/\n##\s+References\s*\n+([\s\S]*?)(?=\n##\s|$)/i);
  if (refSection) {
  ...
  return { summary, experience, projects, projectsTitle, skills, education, certifications, languages, references };
}
...
function adaptJobRole(baseRole, company, jdKeywords) {
  ...
  if (company === 'Fiesta Fresh Cleaning') {
...
function adaptProjectName(baseName, jdKeywords) {
  ...
  if (baseName.includes('APEX')) {
    if (kws.includes('qa') || kws.includes('test') || kws.includes('audit')) return 'Fiesta Fresh Cleaning — Automated Website Audit & QA Engine';
    if (kws.includes('voice') || kws.includes('vapi')) return 'Fiesta Fresh Cleaning — AI Voice Calling & Lead Gen Engine';
    if (kws.includes('growth') || kws.includes('b2b') || kws.includes('lead')) return 'Fiesta Fresh Cleaning — B2B Lead Gen & Website Regeneration Engine';
    return 'Fiesta Fresh Cleaning — B2B Automation & Website Regeneration Engine';
  }
 ...
 */
/**
 * @param overrides Optional LLM-written copy: { summary }. Only the summary can
 *   be replaced — employment history, dates and metrics stay deterministic,
 *   because a hallucinated job in a CV she actually sends is unacceptable.
 */
export function buildCVPayload(profile, jdText, cvMdPath = null, overrides = null) {
  const cvData = loadCVData(cvMdPath);
  const jdKeywords = extractJDKeywords(jdText);
  const summary = overrides?.summary || aggressivelyInjectKeywords(buildTailoredSummary(jdKeywords), jdKeywords);
  ...
  return {
    ...
    candidate: {
      ...
      // Deliberately no `linkedin` entry: it used to be set to the portfolio
      // URL, which printed the same link twice in the contact row.
      ...(profile?.linkedinUrl || profile?.linkedin_url
        ? { linkedin: { url: profile.linkedinUrl || profile.linkedin_url,
                        display: String(profile.linkedinUrl || profile.linkedin_url).replace(/^https?:\/\//, '') } }
        : {}),
      location,
    },
    summary,
 ...
 */
export async function generateCVHtmlAsync(profile, jdText, cvMdPath = null, overrides = null) {
  let payload = buildCVPayload(profile, jdText, cvMdPath, overrides);
...
function renderCVHtml(template, payload) {
  ...
  // Strip markdown emphasis markers and stray separators. cv.md writes
  // "**Bachelor of Marketing**" and "AI Fundamentals, *Google*," so both the
  // bold and italic markers, plus trailing commas, leaked onto the page.
  const demark = (v) => String(v == null ? '' : v)
    .replace(/\*\*/g, '')
    .replace(/\*/g, '')
    .replace(/^\s*[:,]\s*/, '')
    .replace(/\s*,\s*$/, '')
...
${bullets}
  ...
  const education = (payload.education || []).map(e => {
    const orgText = demark(e.org);
    const org = orgText ? ` <span class="edu-org">${escapeHtml(orgText)}</span>` : '';
    return `<div class="edu-item">
    <div class="edu-header">
      <div class="edu-title">${escapeHtml(demark(e.title))}${org}</div>
      <div class="edu-year">${escapeHtml(demark(e.year || ''))}</div>
    </div>
  </div>`;
  }).join('\n  ');
  ...
  const certifications = (payload.certifications || []).map(e => {
    const orgText = demark(e.org);
    const yearText = demark(e.year);
    const org = orgText ? `<span class="cert-org">${escapeHtml(orgText)}</span>` : '';
    const year = yearText ? `<span class="cert-year">${escapeHtml(yearText)}</span>` : '';
    return `<div class="cert-item">
      <span class="cert-title">${escapeHtml(demark(e.title))}</span>
      ${org}
      ${year}
    </div>`;
  }).join('\n    ');
  ...
  const skills = (payload.skills || []).map(c => {
    const catText = demark(c.category);
    const cat = catText ? `<span class="skill-category">${escapeHtml(catText)}:</span> ` : '';
    const rawItems = Array.isArray(c.items) ? c.items.join(', ') : c.items;
    return `    <div class="skill-item">${cat}${escapeHtml(demark(rawItems))}</div>`;
  }).join('\n');

  const skillsHtml = skills ? `<div class="skills-grid">\n${skills}\n  </div>` : '';

  // Languages render as one flowing line, pink bold name + plain level,
  // pipe separated, exactly as in the reference CV.
  const languages = (payload.languages || []).map(l => {
    const name = demark(l.name);
    if (!name) return '';
    const level = demark(l.level || '');
    const lvl = level ? ` (${escapeHtml(level)})` : '';
    return `<span class="lang-name">${escapeHtml(name)}</span>${lvl}`;
  }).filter(Boolean).join(' | ');

  const languagesHtml = languages ? `<div class="lang-item">${languages}</div>` : '';

  // References render one per line, pink bold name then plain detail.
  const references = (payload.references || []).map(r => {
    const name = demark(r.name);
    if (!name) return '';
    // Not using demark() here: it strips a leading comma, and the reference CV
    // reads "Taylor Chorley, Supervisor | ...". Strip only the bold markers,
    // then keep the separator the line already had.
    const detailText = String(r.detail == null ? '' : r.detail)
      .replace(/\*+/g, '')
      .trim();
    const detail = detailText
      ? (detailText.startsWith(',') ? escapeHtml(detailText) : ' ' + escapeHtml(detailText))
      : '';
    return `    <div class="ref-item"><span class="ref-name">${escapeHtml(name)}</span>${detail}</div>`;
  }).filter(Boolean).join('\n');

  const referencesHtml = references ? `<div class="ref-list">\n${references}\n  </div>` : '';
...
${bullets}
  ...
  const subs = {
    ...
    SECTION_EXPERIENCE: escapeHtml(st.experience || 'Professional Experience'),
    ...
    SKILLS: skillsHtml,
    SECTION_LANGUAGES: escapeHtml(st.languages || 'Languages'),
    LANGUAGES: languagesHtml,
    SECTION_REFERENCES: escapeHtml(st.references || 'References'),
...
export async function generateCV(profile, jdText, outputDir, jobSlug, overrides = null) {
  ...
  try {
    const html = await generateCVHtmlAsync(profile, jdText, null, overrides);
