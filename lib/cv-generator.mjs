...
function buildTailoredSummary(jdKeywords) {
  const base = 'AI Automation Specialist, Marketing Engineer, and multi-venture founder with 4+ years of experience designing, coding, and deploying end-to-end automation systems across lead generation, content production, SaaS development, and sales operations. Founded and scaled three fully automated businesses without additional headcount.';
  ...
  return `AI Automation Specialist, Marketing Engineer, and multi-venture founder with 4+ years of experience designing, coding, and deploying end-to-end automation systems with strong expertise in ${kwFormatted}. Founded and scaled three fully automated businesses without additional headcount, delivering scalable architectures, workflow orchestration, and measurable ROI.`;
}
...
function adaptJobRole(baseRole, company, jdKeywords) {
  ...
  if (company === 'Fiesta Fresh Cleaning') {
    if (kws.includes('qa') || kws.includes('test') || kws.includes('quality')) return 'Co-Owner & Automation QA Specialist';
    if (kws.includes('growth') || kws.includes('revops') || kws.includes('sales')) return 'Co-Owner & Growth Automation Engineer';
    if (kws.includes('ai') || kws.includes('agent') || kws.includes('voice')) return 'Co-Owner & AI Systems Specialist';
    if (kws.includes('marketing') || kws.includes('seo') || kws.includes('social')) return 'Co-Owner & Marketing Automation Specialist';
    return 'Co-Owner & Operations Automation Lead';
  }
...
function selectExperience(jdKeywords, experienceList) {
  const scored = experienceList.map(exp => {
    ...
    // Reorder bullets per entry so JD-matched accomplishments appear first
    const tailoredBullets = [...exp.bullets].sort((b1, b2) => {
