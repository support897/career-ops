...
import { readStyleTokens, styleTokensFrom, injectThemeStyle } from '../theme-style.mjs';
 ...
 */
/**
 * @param overrides Optional LLM-written prose: { opening, profile_intro, closing }.
 *   Any field supplied replaces the template's version; anything absent keeps
 *   the deterministic text, so a partial model reply cannot leave a gap.
 */
export function buildCoverLetterPayload(profile, job, jdText, overrides = null) {
  ...
  return {
    ...
    letter: {
      ...
      opening: overrides?.opening || opening,
      profile_intro: overrides?.profile_intro || profileIntro,
      achievements,
      problems_section: '',
      closing: overrides?.closing || closing,
      language_closing,
      footnotes: [
 ...
 */
/**
 * Apply the user's theme tokens to rendered cover-letter HTML.
 *
 * The CV injects its accent colour into the saved HTML; the cover letter used to
 * skip this and only get themed inside the PDF renderer, so the .html on disk,
 * the copy in job_inbox.cover_letter, and the dashboard preview all kept the
 * template's blue default while the CV was pink.
 *
 * Tokens from an explicit `profile.style` object take precedence over
 * config/profile.yml. No tokens => html returned unchanged.
 */
function applyProfileTheme(html, profile) {
  try {
    const profilePath = resolve(__dirname, '..', 'config', 'profile.yml');
    const tokens = { ...readStyleTokens(profilePath), ...styleTokensFrom(profile?.style) };
    if (Object.keys(tokens).length === 0) return html;
    return injectThemeStyle(html, tokens);
  } catch {
    // Theming is cosmetic — never fail document generation over it.
    return html;
  }
}

export function generateCoverLetterHtml(profile, job, jdText, overrides = null) {
  const payload = buildCoverLetterPayload(profile, job, jdText, overrides);
  const templatePath = resolve(__dirname, '..', 'templates', 'cover-letter-template.html');
  const template = readFileSync(templatePath, 'utf-8');
  return applyProfileTheme(renderCoverLetterHtml(template, payload), profile);
}
 ...
 */
export async function generateCoverLetter(profile, job, jdText, outputDir, overrides = null) {
  ...
  try {
    const html = generateCoverLetterHtml(profile, job, jdText, overrides);
    ...
    const textPayload = buildCoverLetterPayload(profile, job, jdText, overrides);
