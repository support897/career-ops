...
export function extractBullets(coverLetter) {
  ...
  if (looksLikeHtml) {
    ...
    // Last resort: the LLM cover-letter path sometimes writes prose with no
    // list at all (Deepgram, 2026-08-18). Its body paragraphs ARE the tailored
    // content, so use them rather than leaving the job with no draft for ever.
    // Greeting and sign-off are dropped; only substantial paragraphs qualify.
    const bodyParas = [...clean.matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/gi)]
      .map((m) => stripTags(m[1]))
      .filter((t) => t.length > 120
        && !/^(dear|hi|hello)\b/i.test(t)
        && !/^(kind regards|best regards|warmest regards|sincerely|thank you)/i.test(t));
    if (bodyParas.length) return bodyParas.slice(0, 3);
    return [];
  }
  const bullets = [];
