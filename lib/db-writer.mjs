/**
 * db-writer.mjs — Write scan results to Postgres for multi-user scanning.
 *
...
/**
 * Write scanned jobs directly to the job_inbox table for the dashboard.
 */
export async function writeScannedJobsToInbox(userId, jobs) {
  const pool = getPool();
  let inserted = 0;
  for (const job of jobs) {
    try {
      ...
      const role = job.title || job.role || 'Position';
      const location = job.location || null;
      const salary = job.salary || null;
      const postedAt = job.postedAt ? new Date(job.postedAt).toISOString() : null;

      await pool.query(
        `INSERT INTO job_inbox (
           user_id, url, company, role, location, compensation, salary, apply_url, posted_at,
           doc_status, job_status, created_at, updated_at
         )
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'pending', 'new', NOW(), NOW())
         ON CONFLICT (user_id, url) DO NOTHING`,
        [userId, url, company, role, location, salary, salary, url, postedAt]
      );
      inserted++;
    } catch (e) {
      // ignore duplicates or single-row errors
    }
  }
  return inserted;
}
...
export async function syncToInbox(userId, job, scoreResult, docs = {}) {
  ...
  await pool.query(
    `INSERT INTO job_inbox (
       user_id, url, company, role, location, compensation, salary, apply_url, posted_at,
       score, score_breakdown, why_match, jd_text, cv_html, cover_letter, cover_letter_html, reference_letter, email_draft,
       doc_status, job_status, gmail_draft_id, generation_method, created_at, updated_at
     )
     ...
     ON CONFLICT (user_id, url) DO UPDATE SET
       ...
       cover_letter_html = COALESCE(EXCLUDED.cover_letter_html, job_inbox.cover_letter_html),
       ...
       generation_method = COALESCE(EXCLUDED.generation_method, job_inbox.generation_method),
       updated_at = NOW()`,
    [
      userId,
      job.url,
      ...
      // Styled HTML, so the dashboard preview and download match the PDF that
      // gets attached to the Gmail draft.
      docs.coverLetterHtml || null,
      docs.referenceLetter || null,
      docs.emailDraft || null,
      docs.cvHtml ? 'ready' : 'pending',
      'evaluated',
      docs.gmailDraftId || null,
      docs.generationMethod || 'keyword'
    ]
  );
}
