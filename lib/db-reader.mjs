/**
 * db-reader.mjs — Read user config from Postgres for multi-user scanning.
 *
...
/**
 * SSL settings for a Postgres URL.
 *
 * Managed providers (Neon, Supabase, RDS) require TLS; a local or private-network
 * Postgres typically has it disabled and rejects the handshake outright. Read the
 * answer off the URL rather than hardcoding either one.
 */
function sslFor(url) {
  if (/sslmode=disable/i.test(url)) return false;
  if (/sslmode=(require|verify-ca|verify-full)/i.test(url)) return { rejectUnauthorized: false };
  const local = /@(localhost|127\.0\.0\.1|\[::1\]|host\.docker\.internal|0\.0\.0\.0)[:/]/i.test(url)
    || /@107\.175\.88\.18[:/]/.test(url);
 ...
 *
 * DEAD CODE as of 2026-08-17: cookie-based scraping was removed entirely. The
 * SEEK and LinkedIn providers now use public endpoints that need no session, and
 * lib/cookie-crypto.mjs (the only decryptor) has been deleted. Kept solely
 * because the platform_settings columns still exist; nothing calls this.
 */
...
export async function buildScanConfigFromDB(userId) {
  ...
  // Build location_filter from profile location + country.
  // Country is detected at onboarding and lets country-aware boards query the
  // right market; location is the free-text city/region the user can override.
  const locationFilter = {
    positive: [profile.location, profile.country].filter(Boolean),
    always_allow: ['remote', 'worldwide', 'global', 'anywhere', 'distributed'],
    negative: [],
  };
  ...
  return {
    ...
    schedules,
    platforms,
    platformEnabled: Object.fromEntries(platforms.map((p) => [p.platform, p.enabled])),
    titleFilter,
    locationFilter,
    salaryFilter,
...
export async function getUserPendingJobs(userId, limit = 10) {
  const pool = getPool();
  // Atomically claim jobs by updating status to 'processing' — prevents duplicate Lambda invocations
  const result = await pool.query(
    `UPDATE jobs SET status = 'processing'
     WHERE id IN (
       SELECT id FROM jobs WHERE user_id = $1 AND status = 'pending'
       ORDER BY score DESC NULLS LAST, created_at DESC
       LIMIT $2
     )
     RETURNING id, user_id, title, company, url, platform, location,
               employment_type, salary, salary_min, salary_max, score, status, description,
               company_email, match_reasons, tags, cover_letter, email_body, email_subject, posted_at`,
    [userId, limit]
  );
  return result.rows.map(row => ({
    id: row.id,
...
/**
 * Get user's minimum score threshold for auto-apply (1-5, default 4).
 */
export async function getUserMinScoreForAutoApply(userId) {
  const pool = getPool();
  const result = await pool.query(
    'SELECT min_score_for_auto_apply FROM users WHERE id = $1',
    [userId]
  );
  if (result.rows.length === 0) return 4;
  return result.rows[0].min_score_for_auto_apply || 4;
}

/**
 * Get user's dashboard score threshold from user_profiles (default 3.0).
 */
export async function getUserScoreThreshold(userId) {
  const pool = getPool();
  const result = await pool.query(
    'SELECT score_threshold FROM user_profiles WHERE user_id = $1',
    [userId]
  );
  if (result.rows.length === 0) return 3.0;
  return result.rows[0].score_threshold || 3.0;
}
...
export function decryptPassword(userId, payload) {
  ...
  if (!ENCRYPTION_KEY) {
    throw new Error('ENCRYPTION_KEY is not set — set it in the environment (VPS/Lambda env or .env). Refusing to decrypt with a fallback key.');
  }
  try {
...
/**
 * Get user API keys for job board providers (Jooble, Findwork, Adzuna).
 * Returns plain object with key values.
 */
...
/**
 * Set user API keys in DB. Admin keys are shared globally.
 */
export async function setUserApiKeys(userId, apiKeys) {
  const pool = getPool();
  await pool.query(
...
/**
 * Get total user count. Used to enforce limits (e.g. Jooble/Adzuna first 50 users).
 */
...
/**
 * Count how many applications a user has submitted today (UTC).
 * Used to enforce free-tier daily limits.
 */
export async function getUserApplicationCountToday(userId) {
  const pool = getPool();
  const result = await pool.query(
    `SELECT COUNT(*)::int AS count
     ...
     WHERE user_id = $1
       ...
       AND applied_at < DATE_TRUNC('day', NOW() AT TIME ZONE 'UTC') + INTERVAL '1 day'`,
    [userId]
  );
...
/**
 * Count how many applications a user has submitted on a specific date (YYYY-MM-DD, UTC).
 */
export async function getUserApplicationCountOnDate(userId, dateString) {
  const pool = getPool();
  const result = await pool.query(
    `SELECT COUNT(*)::int AS count
     FROM applications
     WHERE user_id = $1
       AND applied_at >= $2::date
       AND applied_at < $2::date + INTERVAL '1 day'`,
    [userId, dateString]
  );
  return result.rows[0].count || 0;
}
...
/**
 * Read profile_config for a user from `user_profiles`.
 *
 * This is where the dashboard writes the "Use LLM for CV / Cover Letter /
 * Reference Letter" checkboxes and the accent colour. The local pipeline reads
 * config/profile.yml, which has no llm_docs key at all, so without this the
 * checkboxes were decorative: the code fell through to a hardcoded default
 * that had cover_letter disabled.
 *
 * Note this queries user_profiles, not the `profiles` table getUserProfile()
 * expects — user_profiles is the table that actually exists here.
 */
export async function getProfileConfig(userId) {
  if (!userId) return null;
  try {
    const { rows } = await getPool().query(
      'SELECT profile_config FROM user_profiles WHERE user_id = $1 LIMIT 1',
      [userId]
    );
    return rows[0]?.profile_config || null;
  } catch (e) {
    console.log(`[db-reader] getProfileConfig failed: ${e.message}`);
    return null;
  }
}
